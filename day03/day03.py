...
for bank in input_list:
    ...
    if batteries_indexed[0][0] == len(batteries_indexed)-1:
        ...
        second_largest = max((b for b in batteries_indexed if b[0] > batteries_indexed[1][0]), key=lambda x: x[1])
        bank_joltage = str(largest[1]) + str(second_largest[1])
        print(str(largest[1]) + str(second_largest[1]))
    else:
        largest = batteries_indexed[0]
        second_largest = max((b for b in batteries_indexed if b[0] > batteries_indexed[0][0]), key=lambda x: x[1])
        bank_joltage = str(largest[1]) + str(second_largest[1])
        print(str(largest[1]) + str(second_largest[1]))
...
for bank in input_list:
    ...
    for battery_num in range(battery_chain):
        ...
        valid = [b for b in batteries_indexed if b[0] > last_idx and b[0] <= len(bank) - batteries_needed]
